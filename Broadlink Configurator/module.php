            ...
            if ($device['model'] == 'IR' || $device['model'] == 'RF') {
                foreach ($BroadlinkInstanceIDList as $BroadlinkInstanceID) {
                    if ($device['mac'] == IPS_GetProperty($BroadlinkInstanceID, 'mac')) {
                        ...
                        $this->SendDebug(
                            'Broadlink Config', 'device found: ' . utf8_decode($broadlink_device_name) . ' (' . $BroadlinkInstanceID . ')', 0
                        );
                    } else {
                        // delete
        ...
        if (!empty($devices)) {
            foreach ($devices as $device) {
                ...
                foreach ($commands as $key => $command) {
                    ...
                    $Scriptname    = $key;
                    $command_ident =
                        $this->CreateIdent('Broadlink_Device_' . $this->CreateIdent($name) . '_Command_' . $this->CreateIdent($Scriptname));

...
class BroadlinkGateway extends IPSModule
{
    ...
    {
        $data = ['ident' => $deviceident, 'command' => $command];
        // send to device / children
        $this->SendDataToChildren(json_encode(['DataID' => '{A05B41B1-7478-8E54-296E-17F406FD3876}', 'Buffer' => $data]));
    }
    ...
    protected function SendCommand($command)
    {
        ...
        $json = [];
        $info = ['devtype' => $this->ReadPropertyString('devicetype'), 'name' => json_decode($this->ReadPropertyString('name')), 'mac' => $this->ReadPropertyString('mac'), 'host' => $this->ReadPropertyString('host'), 'model' => $this->ReadPropertyString('model')];
        $json['code'] = -1;
    ...
    protected function UpdateA1($device)
    {
        ...
        $data = ['ident' => $deviceident, 'device' => $device];
        // send to device / children
        $this->SendDataToChildren(json_encode(['DataID' => '{D6AB7ABE-1A40-F949-C5B3-64AEAAB179D8}', 'Buffer' => $data]));
    }
    ...
    protected function SendJSON($data)
    {
        ...
        $this->SendDataToChildren(json_encode(['DataID' => '{A05B41B1-7478-8E54-296E-17F406FD3876}', 'Buffer' => $data])); //  I/O RX GUI
    }

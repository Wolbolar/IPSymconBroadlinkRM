...
class Broadlink
{
	...
	function __construct($h = "", $m = "", $p = 80, $d)
	{
		...
		if (is_array($m)) {
			...
			foreach (array_reverse($mac_str_array) as $value) {
				array_push($this->mac, hexdec($value));
			}
	...
	private static function DiscoverDevices($ip)
	{
		...
		restore_error_handler(); // restore warning
	...
	function send_packet($command, $payload)
	{
		...
		$send_packet = array("packet" => $packet, "response" => $this->byte2array($response));
		return $send_packet;
	}
	...
	public function Auth()
	{
		...
		$send_packet = $this->send_packet(0x65, $payload);
		$response = $send_packet["response"];
		...
		$response = array("response" => $response, "payload" => $payload, "id" => $this->id, "key" => $this->key);
		return $response;
	}
...
class SP2 extends Broadlink
{
	...
	public function Check_Power()
	{
		...
		$send_packet = $this->send_packet(0x6a, $packet);
		$response = $send_packet["response"];
...
class RM extends Broadlink
{
	...
	public function Send_data($data)
	{
		...
		$data = array("response" => $response, "packet" => $packet);
		return $data;
	}
...
class MP1 extends Broadlink
{
	...
	public function Check_Power()
	{
		...
		if ($state = $this->Check_Power_Raw()) {
			...
			$data[0] = boolval($state & 0x01);
			$data[1] = boolval($state & 0x02);
			$data[2] = boolval($state & 0x04);
			$data[3] = boolval($state & 0x08);
